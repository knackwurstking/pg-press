...
func (m *ModificationMigration) MigrateAll() (*MigrationStats, error) {
	...
	if err := m.migrateTroubleReports(stats); err != nil {
		return stats, fmt.Errorf("failed to migrate trouble reports: %v", err)
	}

	// Migrate metal sheets
	if err := m.migrateMetalSheets(stats); err != nil {
		return stats, fmt.Errorf("failed to migrate metal sheets: %v", err)
	}

	// Migrate tools
	if err := m.migrateTools(stats); err != nil {
		return stats, fmt.Errorf("failed to migrate tools: %v", err)
	}
...
func (m *ModificationMigration) migrateTroubleReports(stats *MigrationStats) error {
	...
	if err = rows.Err(); err != nil {
		return fmt.Errorf("error iterating trouble report rows: %v", err)
	}
...
func (m *ModificationMigration) migrateMetalSheets(stats *MigrationStats) error {
	...
	for rows.Next() {
		...
		// Parse old mods
		var oldMods oldmodification.Mods[models.MetalSheetModData]
		if err := json.Unmarshal(modsJSON, &oldMods); err != nil {
	...
	if err = rows.Err(); err != nil {
		return fmt.Errorf("error iterating metal sheet rows: %v", err)
	}
...
func (m *ModificationMigration) addModificationWithTimestampTx(tx *sql.Tx, userID int64, entityType ModificationType, entityID int64, data interface{}, timestamp time.Time) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal modification data: %v", err)
	}
...
func (m *ModificationMigration) migrateTools(stats *MigrationStats) error {
	...
	for rows.Next() {
		...
		// Parse old mods
		var oldMods oldmodification.Mods[models.ToolModData]
		if err := json.Unmarshal(modsJSON, &oldMods); err != nil {
	...
	if err = rows.Err(); err != nil {
		return fmt.Errorf("error iterating tool rows: %v", err)
	}
...
func (m *ModificationMigration) CleanupOldMods(force bool) error {
	...
	if !force {
		...
		if err != nil {
			return fmt.Errorf("verification failed before cleanup: %v", err)
		}
...
func (m *ModificationMigration) VerifyMigration() (*VerificationResult, error) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "no such column: mods") {
			...
			return nil, fmt.Errorf("failed to count old trouble report mods: %v", err)
		}
	}
	...
	if !cleanupCompleted {
		...
		if err != nil {
			if strings.Contains(err.Error(), "no such column: mods") {
				...
				return nil, fmt.Errorf("failed to count old metal sheet mods: %v", err)
			}
		}
	}
	...
	if !cleanupCompleted {
		...
		if err != nil {
			if strings.Contains(err.Error(), "no such column: mods") {
				...
				return nil, fmt.Errorf("failed to count old tool mods: %v", err)
			}
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to count new trouble report modifications: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to count new metal sheet modifications: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to count new tool modifications: %v", err)
	}
...
func (m *ModificationMigration) CheckOldModsExist() (bool, error) {
	...
	for _, table := range tables {
		...
		if err != nil {
			...
			return false, fmt.Errorf("error checking mods column in %s: %v", table, err)
		}
	}
