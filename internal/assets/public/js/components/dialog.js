(function () {
	...
	function getOwnedTriggers(root) {
		...
		return Array.from(
			root.querySelectorAll("[data-tui-dialog-trigger]"),
		).filter((trigger) => !trigger.hasAttribute("data-tui-dialog-target"));
	}
	...
	function ensureDialog(dialog) {
		if (!dialog || dialog.dataset.tuiDialogInitialized === "true")
			return dialog;
	...
	function openDialog(target) {
		console.debug("Opening dialog for target:", target); // TODO: Remove debug log
	...
	function initDialogs(root = document) {
		root.querySelectorAll("[data-tui-dialog]").forEach((dialogRoot) => {
			...
			if (
				dialog.getAttribute("data-tui-dialog-initial-open") === "true"
			) {
